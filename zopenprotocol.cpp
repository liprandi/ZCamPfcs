...
bool ZOpenprotocol::parseResult(const QByteArray& data)
{
    bool ret = false;
    int idx = 20;
...
{
    QMutexLocker locker(&fifoscrews);
    m_data.ok = false;
    if(m_data.angle <= m_data.maxAngle && m_data.angle >= m_data.minAngle)
        if(m_data.torque <= m_data.maxTorque && m_data.torque >= m_data.minTorque)
            m_data.ok = true;
    m_listScrews.append(m_data);
    emit dataReady();
}
bool ZOpenprotocol::getScrewData(ScrewInfo& data)
{
    QMutexLocker locker(&fifoscrews);
    bool ret = !m_listScrews.isEmpty();
    if(ret)
    {
        data = m_listScrews.first();
        m_listScrews.removeFirst();
    }
    return ret;
}
